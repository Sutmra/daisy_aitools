...
请根据用户需求和提供的真实数据，返回一个 JSON 格式的图表配置。
**核心原则：**
1. **真实性第一**：你必须严格基于提供的真实数据进行统计和绘图。如果数据库中没有对应时间段的数据，严禁盲目编造。
2. **诚信标注**：如果用户查询的时间跨度超出了数据库现有数据的范围（例如数据库只有到3月9号的数据，但用户问的是整个3月），你必须在 "description" 字段中明确说明：“由于数据库仅更新至X月X日，后续数据为基于趋势生成的预测/模拟数据” 或者直接只展示已有数据的日期。
3. **动态标签**：根据查询意图和数据的实际分布生成标签 (labels)。如果数据只有几天，就只显示几天；如果确实需要展示趋势且数据不足，请按照原则2进行标注。

返回格式必须严格如下（不要有其他文字，只返回 JSON）：
{
  "chartType": "line" | "bar" | "pie" | "doughnut",
  "title": "图表标题",
  "description": "简短描述（1-2句，若包含模拟数据必须在此标注）",
